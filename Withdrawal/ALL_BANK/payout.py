...
class BankSessionManager:
    ...
    def get_page(self, from_bank: str):
        logger.info(f"Switch check: {self.current_bank} → {from_bank}")

        if self.current_bank == from_bank and self.page:
            return self.page

        if from_bank == "KMA Company Web":
            self.page = BankBot.kma_login(self.p)

        elif from_bank == "TTB Company Web":
            self.page = BankBot.ttb_login(self.p)

        else:
            raise ValueError(f"Unsupported fromBankCode: '{from_bank}'")

        self.current_bank = from_bank
        return self.page

    def run_withdrawal(page, job):
        bank = job["fromBankCode"]

        if bank == "KMA Company Web":
            BankBot.kma_withdrawal(page, job)

        elif bank == "TTB Company Web":
            BankBot.ttb_withdrawal(page, job)

        else:
            raise ValueError(f"Unsupported fromBankCode: '{bank}'")

    def ensure_transfer_page(page, bank):
        try:
            if bank == "KMA Company Web":
                if not page.locator("#ddlBanking").is_visible(timeout=3000):
                    logger.info("🔄 Back to KMA transfer page")
                    page.goto(
                        "https://www.krungsribizonline.com/BAY.KOL.Corp.WebSite/Transfer/Transfer.aspx",
                        wait_until="domcontentloaded"
                    )

            elif bank == "TTB Company Web":
                if not page.locator("#counterparty").is_visible(timeout=3000):
                    logger.info("🔄 Back to TTB transfer page")
                    page.locator("//span[normalize-space()='New payment']").click()

        except Exception as e:
            logger.warning(f"ensure_transfer_page failed: {e}")
    ...
    with sync_playwright() as p:
        ...
        while True:
            ...
            try:
                ...
                page = session.get_page(job["fromBankCode"])
                BankSessionManager.ensure_transfer_page(page, job["fromBankCode"])
                BankSessionManager.run_withdrawal(page, job)
