...
class BankBot(Automation):
    ...
    def kma_read_otp_from_webhook(cls, bank="Krungsri", timeout=60):
        """
        Fetch OTP from test copy.py webhook server.
        Wait until OTP is received.
        """

        url = "http://127.0.0.1:3000/otp/latest"
        end = time.time() + timeout

        while time.time() < end:

            try:
                r = requests.get(url, params={"bank": bank}, timeout=5)
                ...
                if r.status_code == 200:
                    ...
                    if otp_code:
                        print("✅ OTP Received from Webhook:", otp_code)
                        return otp_code
        ...
        raise TimeoutError("❌ OTP not received in time")
