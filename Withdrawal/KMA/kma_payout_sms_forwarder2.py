...
class BankBot(Automation):
    ...
    def kma_withdrawal(cls, page, data):
        ...
        # Run Read OTP Code
        otp = cls.kma_read_otp_from_webhook(bank="Krungsri", timeout=90)

        # Fill OTP Code 
        page.fill("#ctl00_cphSectionData_OTPBox1_txtOTPPassword", otp)
        
        # Delay 0.5 second
        page.wait_for_timeout(5001111)
    ...
    @classmethod
    def kma_read_otp_from_webhook(cls, bank="Krungsri", timeout=60):
        """
        Fetch OTP from webhook by matching the website Ref (cls._kma_ref).
        Safe when multiple phones/OTPs exist.
        """
        url = "http://127.0.0.1:3000/otp/by_ref"
        end = time.time() + timeout

        while time.time() < end:
            try:
                r = requests.get(url, params={"bank": bank, "ref": cls._kma_ref}, timeout=5)

                if r.status_code == 200:
                    otp_data = r.json()
                    otp_code = otp_data.get("otp")
                    if otp_code:
                        print(f"✅ OTP Received | Ref={cls._kma_ref} | OTP={otp_code}")
                        return otp_code

            except Exception as e:
                print("Webhook not ready:", e)

            time.sleep(1)

        raise TimeoutError(f"❌ OTP not received in time for Ref={cls._kma_ref}")
