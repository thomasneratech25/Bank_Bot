...
class BankBot(Automation):
    ...
    def scb_Anywhere_apps(cls, data):
        ...
        device().adb.shell("settings put global adb_enabled 12")

        # Start SCB Corporate Apps
        out = shell("dumpsys window windows") 
        if SCB_APP_PACKAGE not in out:
        ...
        poco("tabNotificationsStack").wait_for_appearance(timeout=30)
        ...
        while True:
            if not poco(text="Pending edit (0)").exists():
                poco(text="Approved").wait_for_appearance(timeout=1)
                poco("btTodoList").wait_for_appearance(timeout=1)
