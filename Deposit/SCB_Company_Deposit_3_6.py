...
class Bank_Bot(Automation):
    ...
    def detect_new_transactions(cls, page):
        ...
        # Send ALL transactions on first run
        if not history:
            print("⚡ First run detected — sending ALL transactions")

            # Save newest as baseline
            cls.save_last_seen(tx_list[0])

            # Return oldest → newest
            return list(reversed(tx_list))

        # ================= NORMAL RUN =================
        for tx in tx_list:

            # Skip malformed rows
            if not tx.strip() or "|" not in tx:
                continue

            # Stop ONLY when the newest known tx is reached
            if tx == history[0]:
                break
        ...
        # Update last seen with newest tx
        cls.save_last_seen(tx_list[0])
        # Return oldest -> newest
        ...
        with sync_playwright() as p: 
            ...
            while True:
                ...
                try:
                    ...
                    logger.exception("Minor loop error recovered inside transaction loop")
                    time.sleep(1)
                    continue
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            logger.exception("Fatal error — restarting in 10s")
